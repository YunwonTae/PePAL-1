...
def _uid_tokenize(string, truncate=None):
    """Split apart word features (like POS/NER tags) from the tokens.
...
def _onmt_tokenize(string, tokenizer=None):
    tokens=tokenizer.EncodeAsPieces(string)
    return tokens
    ...
    def preprocess(self, x):
        ...
        Returns:
            List[List[str]]: A list of length ``len(self.fields)`` containing
                ...
                is ordered like ``self.fields``.
        """
        # if self.fields[0][0]=="uid":    
        #     return [int(u_str) for u_str in x]
...
def text_fields(**kwargs):
    ...
    if bert is not None:
        ...
        #     prep_ = partial(prep)
        #     uids = Field(
        #         use_vocab=False, tokenize=prep_,
        #         dtype=torch.long)
        #     fields_.append(('uid', uids))

    else:
        for i in range(n_feats + 1):
            name = base_name + "_feat_" + str(i - 1) if i > 0 else base_name
            tokenizer = spm.SentencePieceProcessor()
            tokenizer.Load('/home/irteam/users/kaist/dataset/wmt-ende/sentencepiece.model')
            # tokenize = partial(_uid_tokenize,truncate=None)
            tokenize = partial(
                _onmt_tokenize,
                tokenizer=tokenizer)
#             import pdb;
