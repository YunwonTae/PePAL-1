...
def model_opts(parser):
    ...
    group.add('--user_emb', '-user_emb', action="store_true",
              help='add user embedding.')
    ...
    group.add('--decoder_type', '-decoder_type', type=str, default='rnn',
              ...
              help="Type of decoder layer to use. Non-RNN layers "
                   "are experimental. Options are "
                   "[rnn|transformer|cnn].")
    group.add('--user_bias','-user_bias', type=str, default='none',
               choices=['factor_cell','full_bias','none'],
               help="Type of user parameter to use" )
    group.add('--user_len','-user_len', type=int, default=9,
               help="number of the user" )
    group.add('--voc_len','-voc_len', type=int, default=119547,
               help="number of the vocab" )
    group.add('--fact_len','-fact_len', type=int, default=10,
               help="user factor factorization dimension" )
    group.add('--user_classify','-user_classify', action='store_true',
               help="add user classification head" )
    group.add('--domain_classify','-domain_classify', action='store_true',
               help="add user classification head" )
    group.add('--domain_adv','-domain_adv', action='store_true',
               help="add user classification head" )
    group.add('--domain_ratio','-domain_ratio', type=float, default=1.0,
               help="add user classification head" )
    group.add('--domain_len','-domain_len', type=int, default=5,
               help="number of the domain" )
    group.add('--user_cls','-user_cls', action='store_true',
               help="add user cls token for classification" )
    group.add('--dom_cls','-dom_cls', action='store_true',
              help="user bias type")
    group.add('--encoder_freeze','-enc_freeze', action='store_true',
              help="encoder freeze option")
    group.add('--domain_cls_enc','-domain_cls_enc', action='store_true',
              help="domain_cls_enc")
    group.add('--dom_avg_pool','-dom_avg_pool', action='store_true',
              help="domain_cls_enc")
...
def preprocess_opts(parser):
    ...
    group.add('--train_uid', '-train_uid',
              help="Path to the train uid data")
    group.add('--valid_uid', '-valid_uid',
              help="Path to the validation uid data")
    group.add('--train_dom', '-train_dom',
              help="Path to the train uid data")
    group.add('--valid_dom', '-valid_dom',
              help="Path to the validation uid data")
...
def train_opts(parser):
    ...
    group.add('--pretrain_from', '-pretrain_from', default='', type=str,
              help="If training from a checkpoint then this is the "
                   "path to the pretrained model's state_dict.")
...
#               help="Share source and target vocabulary")
    ...
    group.add('--beam_size', '-beam_size', type=int, default=3,
              help='Beam size')
...
def translate_opts(parser):
    ...
    group.add('--src', '-src', required=True,
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--uid', '-uid', required=False,
              help="User id sequence to decode (one line per "
                   "sequence)")
    group.add('--dom', '-dom', required=False,
              help="User id sequence to decode (one line per "
                   "sequence)")
    group.add('--src_dir', '-src_dir', default="",
              help='Source directory for image or audio files')
