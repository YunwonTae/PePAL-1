...
def train_build_translator(opt, model, model_opt, fields, report_score=True, logger=None, out_file=None):
...
class Translator(object):
    ...
    def from_opt(
        ...
        uid_reader = inputters.str2reader["text"].from_opt(opt)
        dom_reader = inputters.str2reader["text"].from_opt(opt)
        ...
        return cls(
            ...
            user_cls=opt.user_cls,
            dom_cls=opt.dom_cls,
            user_emb=opt.user_emb)
    ...
    def translate(
        ...
        data = inputters.Dataset(
            ...
            readers=([self.src_reader, self.tgt_reader]
                     if tgt else [self.src_reader, self.uid_reader, self.dom_reader]),
            data=[("src", src), ("tgt", tgt)] if tgt else [("src", src),("uid",uid),("dom",dom)],
            dirs=[src_dir, None] if tgt else [src_dir, src_dir, src_dir],
...
#         pdb.set_trace()
    ...
    def _run_encoder(self, batch):
        ...
        if self.user_cls:
            ...
            pre_enc_state, memory_bank, src_lengths = self.model.encoder(src, src_lengths)
            enc_states = pre_enc_state[:,1:,:]
            cls_states = pre_enc_state[:,0,:]
    ...
    def _decode_and_generate(
        ...
        if self.user_emb:
            if batch_indices != None:
                uid = batch.uid[batch_indices].unsqueeze(-1).expand_as(decoder_in.view(3,-1,1)).reshape(-1)
                dec_out, dec_attn = self.model.decoder(
                    decoder_in, memory_bank, memory_lengths=memory_lengths, step=step, uid=uid
                )
            else:
                uid = batch.uid.unsqueeze(-1).expand_as(decoder_in.view(3,-1,1)).reshape(-1)
                dec_out, dec_attn = self.model.decoder(
                    decoder_in, memory_bank, memory_lengths=memory_lengths, step=step, uid=uid
                )
        else:
...
#                 print(log_probs.shape)
                ...
                if self.user_bias=='factor_cell':
                    if batch_indices != None:
                        out = out + torch.matmul(self.model.user_bias(batch.uid[batch_indices]),self.model.user_global)
                    else:
                        out = out + torch.matmul(self.model.user_bias(batch.uid),self.model.user_global)
                else:
#                     import pdb
#                     pdb.set_trace()
                    if batch_indices != None:
                        out = out + self.model.user_bias(batch.uid[batch_indices])
                    else:
...
#                 import pdb
#                 pdb.set_trace()
# torch.sum(beam.is_finished,dim=1)
                beam_out = beam.update_finished()
                if beam.done:
                    break

            select_indices = beam.current_origin
#             print(beam.is_finished)
            if any_beam_is_finished:
                # Reorder states.
                if isinstance(memory_bank, tuple):
                    memory_bank = tuple(x.index_select(1, select_indices)
