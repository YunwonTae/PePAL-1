...
def build_report_manager(opt):
    ...
    report_mgr = ReportMgr(opt, opt.report_every, start_time=-1,
...
class ReportMgr(ReportMgrBase):
    def __init__(self, opt, report_every, start_time=-1., tensorboard_writer=None):
        """
    ...
    def _report_step(self, lr, step, train_stats=None, valid_stats=None):
        ...
        if valid_stats is not None:
            ...
            if self.opt.domain_cls_enc == False:
                self.log('Validation bleu score:%g' % valid_stats.avg_bleu())
                self.log('Validation ter score:%g' % valid_stats.avg_ter())
